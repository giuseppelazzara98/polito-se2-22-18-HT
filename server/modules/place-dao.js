class PlaceDAO {
	...
	newPlaceTable = () => {
		return new Promise((resolve, reject) => {
			const sql =
				'CREATE TABLE IF NOT EXISTS PLACE(id_place INTEGER NOT NULL, id_province INTEGER, description TEXT, latitude REAL NOT NULL, longitude NUMERIC NOT NULL, type BLOB NOT NULL, PRIMARY KEY(id_place AUTOINCREMENT), FOREIGN KEY(id_province) REFERENCES PROVINCE(id_province));';
			this.db.run(sql, (err) => {
				if (err) {
